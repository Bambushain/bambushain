...
pub struct MountBroadcaster {
    inner: Mutex<MountBroadcasterInner>,
}

#[derive(Debug, Clone, Default)]
struct MountBroadcasterInner {
    clients: Vec<actix_web_lab::sse::Sender>,
}

impl MountBroadcaster {
    pub fn create() -> Arc<Self> {
        let this = Arc::new(MountBroadcaster {
            inner: Mutex::new(MountBroadcasterInner::default()),
        });
        MountBroadcaster::spawn_ping(Arc::clone(&this));
    ...
    async fn remove_stale_clients(&self) {
        ...
        log::info!("Active mount client {:?}", clients);
        ...
        log::info!("Okay mount active client {:?}", ok_clients);

        self.inner.lock().clients = ok_clients;
    }

    pub async fn new_client(&self) -> Sse<ChannelStream> {
        log::info!("Starting creation of mount broadcaster");
