...
pub struct KillBroadcaster {
    inner: Mutex<KillBroadcasterInner>,
}

#[derive(Debug, Clone, Default)]
struct KillBroadcasterInner {
    clients: Vec<actix_web_lab::sse::Sender>,
}

impl KillBroadcaster {
    pub fn create() -> Arc<Self> {
        let this = Arc::new(KillBroadcaster {
            inner: Mutex::new(KillBroadcasterInner::default()),
        });
        KillBroadcaster::spawn_ping(Arc::clone(&this));
    ...
    async fn remove_stale_clients(&self) {
        ...
        log::info!("Active kill client {:?}", clients);
        ...
        log::info!("Okay kill active client {:?}", ok_clients);

        self.inner.lock().clients = ok_clients;
    }

    pub async fn new_client(&self) -> Sse<ChannelStream> {
        log::info!("Starting creation of kill broadcaster");
