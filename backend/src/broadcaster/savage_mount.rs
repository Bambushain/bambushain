...
pub struct SavageMountBroadcaster {
    inner: Mutex<SavageMountBroadcasterInner>,
}

#[derive(Debug, Clone, Default)]
struct SavageMountBroadcasterInner {
    clients: Vec<actix_web_lab::sse::Sender>,
}

impl SavageMountBroadcaster {
    pub fn create() -> Arc<Self> {
        let this = Arc::new(SavageMountBroadcaster {
            inner: Mutex::new(SavageMountBroadcasterInner::default()),
        });
        SavageMountBroadcaster::spawn_ping(Arc::clone(&this));
    ...
    async fn remove_stale_clients(&self) {
        ...
        log::info!("Active savage mount client {:?}", clients);
        ...
        log::info!("Okay savage mount active client {:?}", ok_clients);

        self.inner.lock().clients = ok_clients;
    }

    pub async fn new_client(&self) -> Sse<ChannelStream> {
        log::info!("Starting creation of savage mount broadcaster");
