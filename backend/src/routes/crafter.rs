...
pub async fn create_crafter(body: web::Json<Crafter>, authentication: Authentication, db: DbConnection) -> HttpResponse {
    if pandaparty_dbal::crafter::crafter_exists_by_job(authentication.user.id, body.job.clone(), &db).await {
...
pub async fn update_crafter(body: web::Json<Crafter>, path: web::Path<CrafterPathInfo>, db: DbConnection) -> HttpResponse {
