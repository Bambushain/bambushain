...
pub async fn get_users() -> HttpResponse {
    ok_or_error!(sheef_dbal::user::get_users().await.map(|users| users.into_iter().map(|u| u.to_web_user()).collect::<Vec<WebUser>>()))
}

pub async fn get_user(info: web::Path<UserPathInfo>) -> HttpResponse {
    ok_or_error!(sheef_dbal::user::get_user(info.username.clone()).await.map(|u| u.to_web_user()))
}
...
pub async fn create_user(user: web::Json<User>, notification_state: web::Data<NotificationState>) -> HttpResponse {
    ...
    let data = sheef_dbal::user::create_user(user.into_inner()).await.map(|u| u.to_web_user());
...
pub async fn delete_user(info: web::Path<UserPathInfo>, notification_state: web::Data<NotificationState>, req: HttpRequest) -> HttpResponse {
    ...
    let data = sheef_dbal::user::delete_user(info.username.clone()).await;
    actix_web::rt::spawn(async move {
        notification_state.crew_broadcaster.notify_change().await;
    });
...
pub async fn change_password(info: web::Path<UserPathInfo>, body: web::Json<ChangePassword>, req: HttpRequest) -> HttpResponse {
    ...
    no_content_or_error!(sheef_dbal::user::change_password(info.username.clone(), body.new_password.clone()).await)
}

pub async fn change_my_password(body: web::Json<ChangeMyPassword>, req: HttpRequest) -> HttpResponse {
    let username = username!(req);
    match sheef_dbal::user::change_my_password(username.clone(), body.old_password.clone(), body.new_password.clone()).await {
        Ok(_) => no_content!(),
