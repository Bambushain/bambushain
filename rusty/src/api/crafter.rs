...
async fn get_crafter() -> Result<Vec<sheef_entities::Crafter>, ErrorCode> {
...
pub async fn create_crafter(crafter: sheef_entities::Crafter) -> Result<sheef_entities::Crafter, ErrorCode> {
    log::debug!("Create crafter {}", crafter.job);
    post("/api/crafter", &crafter).await
}

pub async fn update_crafter(job: String, crafter: sheef_entities::Crafter) -> ErrorCode {
    log::debug!("Create crafter {}", crafter.job);
    put(format!("/api/crafter/{}", job), &crafter).await
}

pub async fn delete_crafter(crafter: sheef_entities::Crafter) -> ErrorCode {
