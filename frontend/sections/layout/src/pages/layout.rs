...
fn update_my_profile_dialog(props: &UpdateMyProfileDialogProps) -> Html {
    ...
    let on_save = {
        ...
        Callback::from(move |_| {
            ...
            yew::platform::spawn_local(async move {
                error_state.set(match update_my_profile(UpdateProfile::new((*email_state).to_string(), (*display_name_state).to_string(), (*discord_name_state).to_string())).await {
                    Ok(_) => {
            ...
            yew::platform::spawn_local(async move {
                ...
                .await
                {
                    ...
                    Err(err) => {
                        ...
                        error_state.set(true);
                        if err.code > ErrorCode::from(499) {
                            error_message_state
