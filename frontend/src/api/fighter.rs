...
async fn get_fighter(character_id: i32) -> PandapartyApiResult<Vec<Fighter>> {
    ...
    get(format!("/api/final-fantasy/character/{character_id}/fighter")).await
}
...
impl Query for FighterForCharacter {
    ...
    async fn query(_states: &BounceStates, input: Rc<Self::Input>) -> QueryResult<Self> {
        get_fighter(*input).await.map(|fighter| Rc::new(fighter.into()))
    }
}
...
pub async fn create_fighter(character_id: i32, fighter: Fighter) -> PandapartyApiResult<Fighter> {
    ...
    post(format!("/api/final-fantasy/character/{character_id}/fighter"), &fighter).await
}

pub async fn update_fighter(character_id: i32, id: i32, fighter: Fighter) -> PandapartyApiResult<()> {
    log::debug!("Update fighter {id}");
    put_no_content(format!("/api/final-fantasy/character/{character_id}/fighter/{id}"), &fighter).await
}

pub async fn delete_fighter(character_id: i32, id: i32) -> PandapartyApiResult<()> {
    log::debug!("Delete fighter {id}");
    delete(format!("/api/final-fantasy/character/{character_id}/fighter/{id}")).await
}